...
function LeagueDetails() {
  ...
  if (!id) {
    return (
      <div className="rounded-xl border border-zinc-800 bg-zinc-900">
        <EmptyState icon={Trophy} title="Лига не найдена" text="Откройте лигу из списка, чтобы увидеть таблицу и матчи." />
      </div>
    );
  }
  ...
  const allTabs = [
    { key: "info",     icon: Info,         label: "Инфо" },
    { key: "table",    icon: ListOrdered,  label: "Таблица" },
    { key: "schedule", icon: CalendarDays, label: "Матчи" },
    ...(user ? [
      { key: "my",      icon: Users,        label: "Мои" },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-0.5">Лига</p>
          <h1 className="text-xl font-bold text-zinc-100 truncate">{league?.name || "Лига"}</h1>
      ...
      {tab === "info" && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 divide-y divide-zinc-800">
          {[
            { icon: BarChart2,   label: "Формат",     value: league?.rounds_type === "double" ? "Двойной круг" : "Один круг" },
            { icon: Users,       label: "Максимум",   value: `${league?.max_players ?? "—"} игроков` },
            { icon: ListOrdered, label: "Участников", value: String(standings.length) },
          ].map((row) => (
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <span className="flex-1 text-sm text-zinc-400">Статус</span>
            {league && <LeagueStatusBadge status={league.status} />}
          </div>
        </div>
      )}
      ...
      {tab === "table" && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
          {standings.length === 0 ? (
            <EmptyState icon={ListOrdered} title="Таблица пустая" text="Таблица появится после одобрения участников." />
          ) : (
      ...
      {tab === "schedule" && (
        <div className="space-y-3">
          {rounds.length === 0 ? (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900">
              <EmptyState icon={CalendarDays} title="Расписание еще не создано" text="Жеребьевка еще не проведена." />
            </div>
          ) : (
            rounds.map((round) => (
              <div key={round.round} className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800">
                  <span className="text-sm font-semibold text-zinc-300">{round.round} тур</span>
                  <span className="text-xs text-zinc-600">
                    {round.matches.length} матч{round.matches.length === 1 ? "" : "а"}
                  </span>
                </div>
      ...
      {tab === "my" && (
        <div className="space-y-3">
          {myMatches.length === 0 ? (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900">
              <EmptyState icon={Users} title="У вас нет активных матчей" text="Матчи ожидающие действий появятся здесь." />
            </div>
          ) : (
      ...
      {tab === "history" && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
          {history.length === 0 ? (
            <EmptyState icon={History} title="История пока пустая" text="Подтвержденные матчи будут здесь." />
          ) : (
            <div>
              {history.map((match) => (
